...
# 生成HTML文件
test_dir = '/Users/xuzhen/PycharmProjects/EUV_apiTest'
report_dir = '/Users/xuzhen/PycharmProjects/EUV_apiTest/report'
discover = unittest.defaultTestLoader.discover(test_dir, pattern='test*.py')
now = time.strftime("%Y-%m-%d %H_%M_%S")
filename = report_dir + '/' + now + 'result.html'
fp = open(filename, 'wb')
runner = HTMLTestRunner(stream=fp, title='EUV-BS端接口自动化测试报告', description='用例执行情况：')
runner.run(discover)
fp.close()
...
# 发送邮件主题
subject = 'EUV API AutoTest email'
